  ...
  return (
    <>
      <div className="flex flex-row flex-wrap">
        {/* For Loop */}
        {data?.map((product) =>  <div className="shadow-lg hover:shadow-2xl p-4 w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 md:p-2 md:mb-4 duration-300 transition-all" key={product._id}>
              {/* Card */}
              <div className="product py-2 px-4">

                {/* Imaage and to Product Details */}
                <Link to={`/ProductDetails/${product.id}/${product.category.name}`}>
                <img src={product.imageCover} className="w-full" alt={product.title} loading="lazy"/>
                </Link>
